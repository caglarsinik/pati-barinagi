...
export function placeEgg(sim: Sim, building: Building, eggId: number): IncubatorResult {
  if (building.type !== 'incubator') return { ok: false, message: 'Bu bir kuluçka değil' };
  if (!isReady(building)) return { ok: false, message: 'Kuluçka henüz inşa ediliyor' };
  if (building.eggs.length >= incubatorSlots(building)) return { ok: false, message: 'Kuluçkada boş yuva yok' };
  const idx = sim.backpack.findIndex((e) => e.id === eggId);
  if (idx === -1) return { ok: false, message: 'Yumurta çantada değil' };
  const egg = sim.backpack.splice(idx, 1)[0];
  egg.hatchLeft = hatchMinutes();
  building.eggs.push(egg);
  return { ok: true, message: 'Yumurta kuluçkaya kondu' };
}
...
export function takeEgg(sim: Sim, building: Building, eggId: number): IncubatorResult {
  ...
  if (sim.backpack.length >= sim.backpackSlots()) return { ok: false, message: 'Çanta dolu' };
...
export function tickIncubators(sim: Sim, dtMin: number): void {
  for (const b of sim.buildings) {
    ...
    for (const egg of hatched) {
      ...
      sim.events.emit('message', `Yumurta çatladı: ${dog.name} doğdu!`);
    }
  }
}
